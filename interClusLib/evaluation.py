...
class Evaluation:
    ...
    def silhouette_score(data, labels, metric) -> float:
        ...
        if metric in sim_funcs_md:
            distance_func = sim_funcs_md[metric]
            is_sim = True
        elif metric in dis_funcs_md:
            distance_func = dis_funcs_md[metric]
            is_sim = False
        else:
            valid_metric = ", ".join(list(sim_funcs_md.keys()) + list(dis_funcs_md.keys()))
